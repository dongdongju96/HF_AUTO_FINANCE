...
class DealerTrackAutomation:
    ...
    def setup_file_path(self):
        ...
        # self.file_name = os.path.join("C:\Users\HF\Desktop\airtable\TDauto\airtable_data", f"table_list_{current_date}.json")
    ...
    def fill_and_submit_form(self):
        ...
        self.fill_dropdown("ddLenders", "Royal Bank of Canada")
    ...
    def select_street_type_cur_employment(self):
        ...
        # Street Type 필드가 존재할 때만 실행
        if "Employer Street Type" in self.data["fields"] and self.data["fields"]["Employer Street Type"]:
    ...
    def select_vehicle_condition(self):
        # ctl21_ctl20_ctl00_ddlVehicleType
        vehicle_condition_selection_dropdown = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable((By.ID, "ctl21_ctl20_ctl00_ddlVehicleType"))
        )
        time.sleep(0.5)

        # Create Select object
        select = Select(vehicle_condition_selection_dropdown)
        select.select_by_visible_text("Used")

    def select_pri_vehicle_use(self):
        vehicle_use_selection_dropdown = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable((By.ID, "ctl21_ctl20_ctl00_ddlVehicleUse"))
        )
        time.sleep(0.5)

        # Create Select object
        select = Select(vehicle_use_selection_dropdown)
        select.select_by_visible_text("Personal")
    
    def check_vehicle_retailed(self):
        checkbox = self.driver.find_element(By.ID, "ctl21_ctl20_ctl00_chkYesNoVehicleRetailed_0")
    ...
    def select_interest_rate(self):
        ...
        while True:
            ...
            if not options:
                ...
                if program_selection_value == "Consumer, BVS and Newcomer Rates":
                ...
                standard_option = "Consumer, BVS and Newcomer Rates"
    ...
    def run(self, airtable_client, client_data_id):
        try:
            ...
            self.select_vehicle_condition()

            self.check_vehicle_retailed()

            self.select_pri_vehicle_use()
